...
export async function GET(request: Request, { params }: RouteProps) {
  try {
    ...
    const printPath = `/${locale}/teacher-tools/variants/${id}/print${forwardParams.toString() ? `?${forwardParams.toString()}` : ""}`;
    const response = await renderPdfFromPrintPath(request, printPath);
    if (response.status === 200) {
      response.headers.set("Content-Disposition", `attachment; filename="variant-${id}.pdf"`);
    }
    return response;
  } catch (error) {
