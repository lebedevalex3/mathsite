...
export async function GET(request: Request, { params }: RouteProps) {
  try {
    ...
    const queryLayoutRaw = requestUrl.searchParams.get("layout");
    const queryLayout = queryLayoutRaw ? parsePrintLayout(queryLayoutRaw) : null;
    ...
    const selectedEngine = selectWorkPdfEngine({
      ...
      layout: queryLayout ?? storedProfile.layout,
    ...
    if (isDev) {
      console.log({
        ...
        layout: queryLayout ?? storedProfile.layout,
        orientation:
          requestUrl.searchParams.get("orientation") ??
          (queryLayout ? defaultOrientationForLayout(queryLayout) : storedProfile.orientation),
      });
    }
    ...
    if (selectedEngine.engine === "latex") {
      ...
      const effectiveLayout = queryLayout ?? doc.profile.layout;
      const effectiveOrientation = parsePrintOrientation(
        requestUrl.searchParams.get("orientation"),
        queryLayout ? defaultOrientationForLayout(queryLayout) : doc.profile.orientation,
      );
      ...
      try {
        const pdf = await latexWorkPdfEngine.renderStudentPdf(latexDoc);
        return new Response(Buffer.from(pdf), {
          status: 200,
          headers: {
            "Content-Type": "application/pdf",
            "Cache-Control": "no-store",
            "Content-Disposition": `attachment; filename="work-${id}.pdf"`,
            "X-PDF-Engine": "latex",
            "X-PDF-Engine-Source": selectedEngine.source,
          },
        });
      } catch (error) {
        if (error instanceof Error && /LATEX_UNSUPPORTED_PROFILE/.test(error.message)) {
          return NextResponse.json(
            {
              ok: false,
              code: "LATEX_UNSUPPORTED_PROFILE",
              message:
                "LaTeX PDF beta could not render this profile. For two_cut, use short variants or switch to a lighter layout.",
            },
            {
              status: 501,
              headers: {
              ...
              ...(process.env.NODE_ENV !== "production"
                ? {
                    hints: [
                      ...
                      "Use layout=single, layout=two, layout=two_dup, or layout=two_cut (short variants only) for LaTeX beta.",
    ...
    const printPath = `/${locale}/teacher-tools/works/${id}/print${forwardParams.toString() ? `?${forwardParams.toString()}` : ""}`;
    ...
    if (response.status === 200) {
      response.headers.set("Content-Disposition", `attachment; filename="work-${id}.pdf"`);
    }
    return response;
  } catch (error) {
    const { status, body } = toApiError(error, {
      defaultCode: "PDF_ERROR",
      defaultMessage: "Failed to export PDF.",
    });
    return NextResponse.json(body, { status });
  }
}
